...
const objLat = (obj) => {
    ...
    function containsUppercase(name) {
        if (/[A-Z]/.test(name)){
            return name;
        }else{
            const Cname = name.charAt(0).toUpperCase()+ name.substring(1);
            return Cname;
         }
            
    };
    
    return `my name is ${containsUppercase(obj.firstName)} ${containsUppercase(obj.lastName)} I am ${obj.age} YO, and I love ${obj.hobby}.`;
};
// -------------------------------------------------------------------------------------------------------
...
const cvFormatter = (arr) => {
    ...
    const output = arr.filter( user => user.yearsOfExperience > 1 );
   ...
   for (let object = 0;object< output.length ; object ++){
     if (output[object].lastName !== null){
       ...
       let fullName = output[object].firstName;
       let tech = output[object].tech;
         finalArray.push({"fullName":fullName,"tech":tech});
       }
  }
